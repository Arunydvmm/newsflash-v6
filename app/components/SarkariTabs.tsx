                ...
                <Link key={String(job._id)} href={`/sarkari/${job.slug}`} style={{ textDecoration: 'none' }}>
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: '#666666', marginBottom: 4 }}>{job.organization}</div>
                      <div style={{ display: 'flex', gap: 12, fontSize: 11, color: '#666666', fontFamily: 'JetBrains Mono, monospace' }}>
              ...
              return (
                <Link key={String(item._id)} href={`/exam-portal/${item._id}`} style={{ textDecoration: 'none' }}>
                  ...
                  >
                    ...
                    <div style={{ fontSize: 12, color: '#666666', marginBottom: 8 }}>{item.organization}</div>
            ...
            {activeTabData.items.slice(0, 6).map((item: any) => (
              <Link key={String(item._id)} href={`/exam-portal/${item._id}`} style={{ textDecoration: 'none' }}>
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: '#666666', marginBottom: 8 }}>{item.organization}</div>
                  <div style={{ fontSize: 11, color: '#666666', marginBottom: 12, fontFamily: 'JetBrains Mono, monospace' }}>📅 {fmt(item.importantDates?.admitCardDate || item.importantDates?.answerKeyDate || item.importantDates?.resultDate)}</div>
