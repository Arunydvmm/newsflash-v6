...
export const metadata: Metadata = {
  title: 'Exam Portal 2026 — Jobs, Exams, Answer Keys, Admit Cards, Results',
  description: 'Latest Government Jobs, Exams, Answer Keys, Admit Cards, Results, Exam Notifications. SSC, UPSC, Railway, Bank, Police jobs & exams 2026.',
  keywords: ['exam portal', 'government jobs', 'exams', 'answer keys', 'admit cards', 'exam results', 'latest jobs 2026'],
}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '600px' }}>
          ...
          <tbody>
            {items.map((item: any, idx: number) => {
              const dl = item.importantDates?.lastDate || item.importantDates?.registrationEnd || item.importantDates?.admitCardDate || item.importantDates?.answerKeyDate || item.importantDates?.resultDate
              ...
              return (
                <tr key={String(item._id)} style={{ borderBottom: '1px solid #E8E8E4', transition: 'background 0.2s', background: idx % 2 === 0 ? 'white' : '#FAFAFA' }} onMouseEnter={(e) => e.currentTarget.style.background = '#F0F0F0'} onMouseLeave={(e) => e.currentTarget.style.background = idx % 2 === 0 ? 'white' : '#FAFAFA'}>
                  ...
                  <td style={{ padding: '14px 16px', fontSize: 13, fontFamily: 'JetBrains Mono, monospace', textAlign: 'center', minWidth: '120px' }}>
                    <div style={{ fontWeight: 600, color: '#0D1B2A' }}>{dl ? format(new Date(dl), 'd MMM yyyy') : '—'}</div>
                    {daysLeft !== null && daysLeft >= 0 && (
                      <div style={{ fontSize: 11, marginTop: 4, color: daysLeft <= 3 ? '#C62828' : '#2E7D32', fontWeight: 600 }}>
...
export default async function ExamPortalPage() {
  ...
  const [sarkariJobs, examNotifications, admitCards, answerKeys, results] = await Promise.all([
    ExamPortal.find({ isActive: true, isExpired: false, type: 'sarkari-job' }).sort({ isFeatured: -1, createdAt: -1 }).limit(50).lean(),
  ...
  const allJobs = [...sarkariJobs, ...examNotifications].sort((a, b) => {
      ...
      <header style={{ background: '#1B5E20', color: 'white' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px' }}>
          ...
          <div style={{ fontFamily: 'Playfair Display, serif', fontSize: 32, fontWeight: 900, marginTop: 8, marginBottom: 4 }}>
            Exam <span style={{ color: '#A5D6A7' }}>Portal</span>
          </div>
          <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.8)' }}>Government Jobs • Exams • Answer Keys • Admit Cards • Results</div>
        </div>
      </header>

      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px' }}>
        <TableSection icon="💼" title="All Jobs & Exams" color="#E65100" items={allJobs} type="vacancy" />
        <TableSection icon="🎫" title="Admit Cards" color="#E65100" items={admitCards} type="admit-cards" />
        <TableSection icon="📝" title="Answer Keys" color="#6A1B9A" items={answerKeys} type="answer-keys" />
        <TableSection icon="✅" title="Results" color="#2E7D32" items={results} type="results" />
      </main>
